...
class MyStories extends React.Component {
	...
	handleSubmit(e){
		...
		.then((body)=>{
			...
			console.log(this.state.stories,'full state obj==========================obj');
		})
						...
						{this.state.myStoryInfo.map((info, index) => {
							...
							return(
								<div key={index} className="stories">
									<div style={{height: '120px', margin: '15px 0px 15px 0px'}} className='col-md-6'>
										<div className="mdc-card">
											<section className="mdc-card__primary">
												<h1 className="mdc-card__title mdc-card__title--large">{info.storyTitle}</h1>
												<h2 className="mdc-card__subtitle">{info.author.authorName}</h2>
											</section>
											<section className="mdc-card__supporting-text">
												{info.sentence}
											</section>
											<section className="mdc-card__actions">
												<button onClick={(e)=>this.handlePickedStory(e, info)}  value={info} className="mdc-button mdc-button--compact mdc-card__action">Edit</button>
